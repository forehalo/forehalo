...
function ProjectCardView({ card, index }: { card: Project; index: number }) {
  ...
  const body = (
    <>
      ...
      <span className="grid size-11 shrink-0 place-items-center self-center">
        <Logo logo={card.logo} name={title} />
      </span>
  ...
  // intro page first; otherwise GitHub, then the live URL when the repo
  // is private (projects.md §cards)
  const href = card.github ?? card.url;
  const link = card.page ? (
    <Link to={card.page} data-cursor="link" className={className}>
      {body}
    </Link>
  ) : (
    <a href={href} target="_blank" rel="noreferrer" data-cursor="link" className={className}>
      {body}
    </a>
  );
